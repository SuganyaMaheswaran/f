...
class MemoryDB {
  ...
  /**
   * Gets a value for the give primaryKey and secondaryKey
  ...
  put(primaryKey, secondaryKey, value) {
    if (!(validateKey(primaryKey) && validateKey(secondaryKey))) {
      throw new Error(
        `primaryKey and secondaryKey strings are required, get primaryKey=${primaryKey}, secondaryKey=${secondaryKey}`
      );
    }
    const db = this.db;
  ...
  /**
   * Queries the list of value (i.e., secondaryKeys) for the given primaryKey.
  ...
  query(primaryKey) {
    if (!validateKey(primaryKey)) {
      throw new Error(`primaryKey string is required, got pimaryKey=${primaryKey}`);
    }
    const db = this.db;
